...
acc = []
for p in Pocketsmith_API.accounts_list:
    acc.append(p["name"])
...
def process_account(message):
    if message.text in acc:
        new_tx["account"] = int(Pocketsmith_API.getIDByName(message.text, "account"))
...
def process_payee_1(message):
    ...
    if message.text not in Pocketsmith_API.payees_list:
        ans = bot.reply_to(
            message,
...
def add_payee(message, param="payee"):
    Pocketsmith_API.addPayee(message.text)
...
def process_payee_2(message):
    if message.text.lower() not in [x.lower() for x in Pocketsmith_API.payees_list]:
        add_payee(message)
        return next_step()
...
def process_category(message):
    cat = []
    for p in Pocketsmith_API.categories_list:
        cat.append(p["name"])
    if message.text in cat:
        new_tx["category"] = Pocketsmith_API.getIDByName(message.text, "category")
...
def process_label(message):
    if (
        message.text in Pocketsmith_API.labels_list
...
def process_related_account(message):
    if message.text in acc:
        new_tx["related_account"] = Pocketsmith_API.getIDByName(message.text, "account")
...
def process_method(message):
    if message.text in Pocketsmith_API.payment_method_list:
...
def process_tax(message):
    if message.text in Pocketsmith_API.tax_list:
...
def getNote():
    new_tx["note"] = Pocketsmith_API.createNote(
        new_tx["category"],
        new_tx["installments"],
        new_tx["third_party"],
...
def process_tx():
    ...
    getNote()
    for cat in Pocketsmith_API.categories_list:
    ...
    r = Pocketsmith_API.uploadTxToPocketsmith(
        new_tx["date"],
        new_tx["payee"],
        installment_amount,
        new_tx["category"],
...
def query_payee(inline_query):
    ...
    try:
        options = []
        i = 1
        for p in Pocketsmith_API.payees_list:  # type: ignore
...
def query_label(inline_query):
    ...
    try:
        options = []
        i = 1
        for p in Pocketsmith_API.labels_list:
            m = re.search(search.lower(), f"{p.lower()}")
            if not isinstance(m, re.Match):
                continue
            if len(m.group()) > 1:
...
def query_payment(inline_query):
    ...
    try:
        options = []
        i = 1
        for p in Pocketsmith_API.payment_method_list:
            m = re.search(search.lower(), f"{p.lower()}")
            if not isinstance(m, re.Match):
                continue
            if len(m.group()) > 1:
...
def query_tax(inline_query):
    ...
    try:
        options = []
        i = 1
        for p in Pocketsmith_API.tax_list:
            m = re.search(search.lower(), f"{p.lower()}")
            if not isinstance(m, re.Match):
                continue
            if len(m.group()) > 1:
